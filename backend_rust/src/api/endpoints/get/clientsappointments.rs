...
impl From <Appointment> for AppointmentModel{
    fn from(a: Appointment)-> Self{
        ...
        Self { id: a_id,  start_time: Some(start_time_f), end_time:  Some(end_time_f), name: a.name, discipline: a.discipline }
    }
}
...
async fn get_all_clients_appointments_for_timerange(
    req: HttpRequest,
    ...
    WHERE a.ClientId = @P1 AND a.Date >=  @P2 AND a.Date <  @P3
    ORDER BY a.Date ASC;"
    );
